...
class Link(models.Model):
	...
	owner = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name="作者")
...
class SideBar(models.Model):
	...
	def content_html(self):
		...
		if self.display_type == self.DISPLAY_HTML:
			...
			context = {'posts':Post.latest_posts(with_related=False)[0:4]}
			result = render_to_string('config/blocks/sidebar_posts.html', context)
		elif self.display_type == self.DISPLAY_HOT:
			context = {'posts':Post.hot_posts()[0:4]}
			result = render_to_string('config/blocks/sidebar_posts.html', context)
		elif self.display_type == self.DISPLAY_COMMENT:
			context = {'comments':Comment.objects.filter(status=Comment.STATUS_NORMAL).order_by('-created_time')[0:4]}
